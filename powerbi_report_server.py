...
from powerbireportserver.api_domain import DataSet, DataSource, MetaData, Report
...
class Constant:
    ...
    EXCEL_WORKBOOK_GET = "EXCEL_WORKBOOK_GET"
    EXTENSIONS = "EXTENSIONS"
    FAVORITE_ITEM_GET = "FAVORITE_ITEM_GET"
    FOLDERS = "FOLDERS"
    KPIS = "KPIS"
    LINKED_REPORTS = "LINKED_REPORTS"
    ME = "ME"
    MOBILE_REPORT_GET = "MOBILE_REPORT_GET"
    POWERBI_REPORT_GET = "POWERBI_REPORT_GET"
...
class PowerBiReportServerAPI:
    ...
    API_ENDPOINTS = {
        Constant.CATALOG_ITEM_GET: "{PBIRS_BASE_URL}/CatalogItems({CATALOG_ID})",
        Constant.DATASET_GET: "{PBIRS_BASE_URL}/Datasets({DATASET_ID})",
        Constant.DATASOURCE_GET: "{PBIRS_BASE_URL}/DataSources({DATASOURCE_ID})",
        Constant.EXCEL_WORKBOOK_GET: "{PBIRS_BASE_URL}/ExcelWorkbooks({EXCEL_WORKBOOK_ID})",
        Constant.EXTENSIONS: "{PBIRS_BASE_URL}/Extensions",
        Constant.FAVORITE_ITEM_GET: "{PBIRS_BASE_URL}/FavoriteItems({FAAVORITE_ITEM_ID})",
        Constant.FOLDERS: "{PBIRS_BASE_URL}/Folders({FOLDER_ID})",
        Constant.KPIS: "{PBIRS_BASE_URL}/Kpis({KPI_ID})",
        Constant.LINKED_REPORTS: "{PBIRS_BASE_URL}/LinkedReports({LINKED_REPORT_ID})",
        Constant.ME: "{PBIRS_BASE_URLL}/Me",
        Constant.MOBILE_REPORT_GET: "{PBIRS_BASE_URL}/MobileReports({MOBILE_REPORT_ID})",
        Constant.POWERBI_REPORT_GET: "{PBIRS_BASE_URL}/PowerBiReports({POWERBI_REPORT_ID})",
        Constant.REPORT_GET: "{PBIRS_BASE_URL}/Reports({REPORT_ID})",
        Constant.RESOURCE_GET: "{PBIRS_BASE_URL}/Resources({RESOURCE_GET})",
        Constant.SESSION: "{PBIRS_BASE_URL}/Session",
        Constant.SUBSCRIPTION_GET: "{PBIRS_BASE_URL}/Subscriptions({SUBSCRIPTION_ID})",
    ...
    #     users: List[PowerBiReportServerAPI.User] = [
    #         PowerBiReportServerAPI.User(
    ...
    def __get_report(self, report_id: str) -> Any:
        """
        Fetch the report from PowerBiReportServer for the given dataset identifier
        """
    ...
    def get_dataset(self, dataset_id: str) -> Any:
        ...
        response = requests.get(url=dataset_get_endpoint, auth=self.__auth)
        # Check if we got response from PowerBi
        if response.status_code != 200:
    ...
    def get_data_source(self, dataset: DataSet) -> Any:
        ...
        print(datasource_get_endpoint)
        # Hit PowerBi
        LOGGER.info("Request to datasource URL={}".format(datasource_get_endpoint))
        response = requests.get(url=datasource_get_endpoint, auth=self.__auth)
...
class Mapper:
    ...
    # def to_datahub_user(
    #     self, user: PowerBiReportServerAPI.User
    # ) -> List[MetadataChangeProposalWrapper]:
    #     """
    ...
    #     self, users: List[PowerBiReportServerAPI.User]
    # ) -> List[MetadataChangeProposalWrapper]:
    #     user_mcps = []
    #
    #     for user in users:
...
class PowerBiReportServerDashboardSource(Source):
    ...
    def __init__(self, config: PowerBiDashboardSourceConfig, ctx: PipelineContext):
        ...
        self.auth = PowerBiReportServerAPI(self.source_config).__auth
