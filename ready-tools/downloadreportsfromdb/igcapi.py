...
class igc:
    ...
    def __init__(self):
        ...
        if proxy == "1":
            s.proxies = {
                "http": os.getenv("PROXY_STRING"),
                "https": os.getenv("PROXY_STRING"),
            }
    ...
    def insert_all_assets(self, asset):
        ...
        r = self.session.post(
            self.base_url + "bundles/assets/", data=asset, headers=headers
        )
        return r
    ...
    def insert_lineage_data(self, lineage):
        headers = {"content-type": "application/xml"}
        r = self.session.post(
            self.base_url + "flows/upload/", data=lineage, headers=headers
        )
        return r
