...
folders_tree = pd.merge(
    folders,
    folders2,
    how="left",
    left_on="folder_parentid",
    right_on="folder_itemid",
    suffixes=("", "_parent"),
)
...
asset_tree = (
    folders_tree.assign(key=1).merge(host.assign(key=1), on="key").drop("key", axis=1)
)
...
# join child and father dataframes, to have both internal ids on a row
asset_tree = pd.merge(
    queries, asset_tree, how="right", left_on="query_reportid", right_on="report_itemid"
)
...
for idx, row in asset_tree[~asset_tree["query_content"].isna()].iterrows():
    if row.report_itemid != query_group:
        ...
        asset_tree.at[idx, "query_name"] = (
            "Query " + row["report_name"] + " " + str(query_counter)
        )
...
asset_tree = pd.merge(
    asset_tree, items, how="left", left_on="query_id", right_on="item_queryid"
)
...
for idx, row in items.iterrows():
    flow_unit = etree.SubElement(
        flow_level, "flowUnit", {"assetID": row.item_internal_id}
    )

    subFlow = etree.SubElement(
        flow_unit,
        "subFlows",
        {
            "flowType": "SYSTEM",
            "comment": "grupo de linhagem da coluna " + row.item_name,
        },
    )

    subFlow_unit = etree.SubElement(
        subFlow,
        "flow",
        {
            "sourceIDs": row.item_name_internal_id,
            "targetIDs": row.item_internal_id,
            "comment": "unidade de linhagem da coluna " + row.item_name,
        },
    )
