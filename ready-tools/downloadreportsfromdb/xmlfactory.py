...
def append_host(parent_level, assets, only_name=False):
    ...
    asset = etree.SubElement(
        ...
        "asset",
        {
            "class": "$PowerBI-PbiServer",
            "repr": host.at[0, "host_name"],
            "ID": host.at[0, "host_internal_id"],
        },
    )
    ...
    if only_name:
        for idx, row in host.iterrows():
            asset.append(
                etree.Element("attribute", {"name": "name", "value": row.host_name})
            )
        ...
        for idx, series in host.iterrows():
            for column in series.keys():
                asset.append(
                    etree.Element(
                        "attribute",
                        {"name": column.split("host_")[1], "value": series[column]},
                    )
                )
...
def append_folders(parent_level, assets, only_name=False):
    ...
    for idx, row in folders.iterrows():
        asset = etree.SubElement(
            ...
            "asset",
            {
                "class": "$PowerBI-PbiFolder",
                "repr": row.folder_name,
                "ID": row.folder_internal_id,
            },
        )
        # create folder attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.folder_name})
        )
        ...
        if len(row.folder_parentid) == 0:
            asset.append(
                etree.Element(
                    ...
                    {
                        "name": "$PbiServer",
                        "assetIDs": assets.at[idx, "host_internal_id"],
                    },
                )
            )
        else:
            asset.append(
                etree.Element(
                    "reference",
                    {"name": "$PbiFolder", "assetIDs": row.folder_internal_id_parent},
                )
            )
    return parent_level
...
def append_reports(parent_level, assets, only_name=False):
    ...
    for idx, row in reports.iterrows():
        asset = etree.SubElement(
            ...
            "asset",
            {
                "class": "$PowerBI-PbiReport",
                "repr": row.report_name,
                "ID": row.report_internal_id,
            },
        )
        # create report attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.report_name})
        )
        # create containment reference
        asset.append(
            etree.Element(
                ...
                {
                    "name": "$PbiFolder",
                    "assetIDs": assets.at[idx, "folder_internal_id"],
                },
            )
        )
    return parent_level
    ...
    if only_name:
        for idx, row in queries.iterrows():
            asset = etree.SubElement(
                ...
                "asset",
                {
                    "class": "$PowerBI-PbiQuery",
                    "repr": row.query_name,
                    "ID": row.query_internal_id,
                },
            )
            # create query attributes
            asset.append(
                etree.Element("attribute", {"name": "name", "value": row.query_name})
            )
            # create containment reference
            asset.append(
                etree.Element(
                    ...
                    {
                        "name": "$PbiReport",
                        "assetIDs": assets.at[idx, "report_internal_id"],
                    },
                )
            )
    else:
        for idx, row in queries.iterrows():
            ...
            asset.append(
                etree.Element(
                    "attribute", {"name": "$query", "value": row.query_content}
                )
            )
            # create containment reference
            asset.append(
...
def append_query_items(parent_level, assets, only_name=False):
    ...
    for idx, row in items.iterrows():
        asset = etree.SubElement(
            ...
            "asset",
            {
                "class": "$PowerBI-PbiQueryItem",
                "repr": row.item_table_name + "." + row.item_name,
                "ID": row.item_internal_id,
            },
        )
        ...
        asset.append(
            etree.Element(
                "attribute",
                {"name": "name", "value": row.item_table_name + "." + row.item_name},
            )
        )
        ...
        asset.append(
            etree.Element(
                "reference",
                {"name": "$PbiQuery", "assetIDs": assets.at[idx, "query_internal_id"]},
            )
        )
...
def append_database_host(parent_level, assets):
    ...
    for idx, row in db_host.iterrows():
        ...
        asset = etree.SubElement(
            parent_level,
            "asset",
            {"class": "host", "repr": hostname, "ID": row.query_host_internal_id},
        )
...
def append_database_instances(parent_level, assets):
    ...
    for idx, row in databases.iterrows():
        ...
        asset = etree.SubElement(
            ...
            {
                "class": "database",
                "repr": row.query_database,
                "ID": row.query_database_internal_id,
            },
        )
        # create database attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.query_database})
        )
        # create containment reference
        asset.append(
            etree.Element(
                "reference",
                {"name": "host", "assetIDs": assets.at[idx, "query_host_internal_id"]},
            )
        )
    ...
    for idx, row in schemas.iterrows():
        ...
        asset = etree.SubElement(
            ...
            "asset",
            {
                ...
                "repr": row.item_table_schema,
                "ID": row.item_table_schema_internal_id,
            },
        )
        # create schema attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.item_table_schema})
        )
        # create containment reference
        asset.append(
            etree.Element(
                ...
                {
                    "name": "database",
                    "assetIDs": assets.at[idx, "query_database_internal_id"],
                },
            )
        )
    ...
    for idx, row in tables.iterrows():
        ...
        asset = etree.SubElement(
            ...
            "asset",
            {
                ...
                "repr": row.item_table_name,
                "ID": row.item_table_name_internal_id,
            },
        )
        # create table attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.item_table_name})
        )
        # create containment reference
        asset.append(
            etree.Element(
                "reference",
                {
                    ...
                    "assetIDs": assets.at[idx, "item_table_schema_internal_id"],
                },
            )
        )
    ...
    for idx, row in columns.iterrows():
        ...
        asset = etree.SubElement(
            ...
            "asset",
            {
                ...
                "repr": row.item_name,
                "ID": row.item_name_internal_id,
            },
        )
        # create columns attributes
        asset.append(
            etree.Element("attribute", {"name": "name", "value": row.item_name})
        )
        # create containment reference
        asset.append(
            etree.Element(
                ...
                {
                    "name": "database_table",
                    "assetIDs": assets.at[idx, "item_table_name_internal_id"],
                },
            )
        )
